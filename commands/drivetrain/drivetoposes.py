...
class DriveToPoses(Command):
    ...
    def execute(self):
        ...
        current_chassis_speed = self.drivetrain.getRobotRelativeChassisSpeeds()

        current_speed = math.hypot(
            current_chassis_speed.vx,
            current_chassis_speed.vy
        )

        xy_mag = abs(
            self.trap_motion_xy.update(
        ...
        vel_rot = -self.trap_motion_rot.update(
            (self.last_goal.rotation() - current_pose.rotation()).radians(),
    ...
    def isWithinLastTolerances(self) -> bool:
        return (
            self.trap_motion_xy.reachedGoal(self.xy_tol_pos_last)
            and self.trap_motion_rot.reachedGoal(self.rot_tol_pos_last)
        )
