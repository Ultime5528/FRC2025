...
class QuestNav:
    def __init__(self):
        ...
        self.command = {"CLEAR": 0, "RESET_HEADING": 1, "RESET_POSE": 2, "PING": 3}
        ...
        self.quest_timestamp = self.nt4Table.getDoubleArrayTopic("timestamp").subscribe(
            [-1.0]
        )

        self.quest_position = self.nt4Table.getFloatArrayTopic("position").subscribe(
            [-1.0, -1.0, -1.0]
        )

        self.quest_quaternion = self.nt4Table.getFloatArrayTopic(
            "quaternion"
        ).subscribe([-1.0, -1.0, -1.0, -1.0])

        self.quest_euler_angles = self.nt4Table.getFloatArrayTopic(
            "eulerAngles"
        ).subscribe([-1.0, -1.0, -1.0])

        self.quest_frame_count = self.nt4Table.getIntegerTopic("frameCount").subscribe(
            -1
        )

        self.quest_battery_percent = self.nt4Table.getDoubleTopic(
            "device/batteryPercent"
        ).subscribe(-1.0)

        self.quest_is_tracking = self.nt4Table.getBooleanTopic(
            "device/isTracking"
        ).subscribe(False)

        self.quest_tracking_lost_count = self.nt4Table.getIntegerTopic(
            "device/trackingLostCounter"
        ).subscribe(-1)

        self.reset_pose_pub = self.nt4Table.getDoubleArrayTopic("resetpose").publish()

        self.heartbeat_request_sub = self.nt4Table.getDoubleTopic(
            "heartbeat/quest_to_robot"
        ).subscribe(-1.0)

        self.heartbeat_response_pub = self.nt4Table.getDoubleTopic(
            "heartbeat/robot_to_quest"
        ).publish()
    ...
    def set_pose(self, pose: Pose2d) -> None:
        ...
        self.reset_pose_pub.set([pose.x, pose.y, pose.rotation().degrees()])
    ...
    def get_connected(self) -> seconds:
        return seconds(
            Timer.getTimestamp() - (self.quest_timestamp.getLastChange() / 1000000)
        ).__lt__(0.25)
